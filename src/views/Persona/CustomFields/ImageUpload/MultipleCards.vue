...
export default {
  ...
  mounted(){
    ...
    console.log("mounted",this.cField)
  },
  methods: {
    addFiles() {
    ...
    onUploadedHandler(file){
      ...
      const fajl= this.cField.files.filter(e=>e.key===file.key).map(el=>el.uploaded=true)
      this.cField.dataSource.push({default:file.fileName,value:file.fileName})
    ...
    makeImageObject(imageFile){
      ...
      console.log(imageObject,this.cField)
      return imageObject
    },
    getImagePreviews() {
